...
class Suscriptor_AuthController extends App_Controller_Action
{
    ...
    public function recuperarClaveAction()
    {
        ...
        if ($usuario != false && $formValid == true) {
            ...
            if ($usuario['rol'] == Application_Form_Login::ROL_SUSCRIPTOR) {
                $s = new Application_Model_Suscriptor();
                $suscriptor = $s->getSlugByUsuarioId($usuario['id']);
                ...
                $a = new Application_Model_Administrador();
                $estab = $e->getEstablecimientoPorUsuario($usuario['id']);
                $admin = $a->getAdministradorxId($usuario['id']);
                if(!empty($estab) && !empty($admin)):
                    $this->_helper->Mail->recuperarContrasenaEstablecimiento(
                        array(
                            'to' => $dataPost['email'],
                            'email' => $dataPost['email'],
                            'nombre' => $admin['nombres'].' '.$admin['apellido_paterno'].' '.$admin['apellido_materno'],
                ...
                //TODO recuperar contraseña para otros
                $a = new Application_Model_Administrador();
                $admin = $a->getAdministradorxId($usuario['id']);
                if(!empty($admin)):
