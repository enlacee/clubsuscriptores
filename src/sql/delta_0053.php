...
/**
 * Description of delta_0053
 *
 * @author phpauldj
 */
class Delta_0053 extends App_Migration_Delta
{
    protected $_autor = 'Paul Taboada';
    protected $_desc = 'Creación de lógica de perfiles y Migración de Data';
...
(3, 'Administrador del Portal', '', 1, 1, '2012-01-05 00:00:00', '2012-01-05 00:00:00', '3');";
        ...
        $sql = $objUser->select()->from('usuario',array('id','rol'));
        $rsuser = $objUser->getAdapter()->fetchAll($sql);
        foreach ($rsuser as $id => $value) {
            $equivale = 
                array('gestor'=>'1', 'establecimiento'=>'2', 'admin'=>'3');
            $userId = $value['id'];
            if ($value['rol']=='gestor' || 
                $value['rol']=='establecimiento' || 
                $value['rol']=='admin') {
                //$objUser->update(array('perfil_id'=> $equivale[$value['rol']]), "id = '".$userId."'");
                $sql = "UPDATE usuario SET perfil_id = ".
                    $equivale[$value['rol']]." WHERE id = '".$userId."';";
                $this->_db->query($sql);
            }
        }
        return true;
