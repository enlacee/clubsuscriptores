        ...
        while ($success) {
            try {
                ...
                if ($fileExists) {
                    include($filePath);
                    $className = sprintf(
                        "%s%0" . self::ZERO_FILL . "d", ucfirst(self::PREFIX), $currentVersion
                    );
                    ...
                    if ($classExists) {
                        ...
                        $this->_db->commit();
                        if ($success) {
                            $msg = "Se Aplico el Delta $currentVersion: " . $delta->getDesc();
                            echo $msg . PHP_EOL;
                            $this->_log->log($msg, Zend_Log::INFO);
                            $this->setVersion($currentVersion++);
                        } else {
                            $msg = "Ha fallado el Delta $currentVersion: " . $delta->getDesc();
                ...
                $this->_db->rollBack();
                echo "Error en el delta $currentVersion: " . $e->getMessage();
            }
        }
